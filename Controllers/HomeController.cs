...
namespace OOAD_PROJECT.Controllers
{
    public class HomeController : Controller
    {
        ...
        DataBaseAccessLayer.data dblayer = new DataBaseAccessLayer.data();
        // GET: Home
        public ActionResult HomeView()
        {
            ...
            TempData.Keep();
            DataSet ds = dblayer.ProductsIndex();            ViewBag.Product = ds.Tables[0];
            return View();
        }
        ...
        {
            DataSet ds = dblayer.MenWearProducts();            ViewBag.Product = ds.Tables[0];
            return View();
        }
        public ActionResult WomenView()
        {
            DataSet ds = dblayer.FemaleWearProducts();            ViewBag.Product = ds.Tables[0];
            return View();
        }
        ...
        public ActionResult Checkout(long ContactNo, string Address,string Email,string Note)
        {
            if(ModelState.IsValid)
            {
                ...
                foreach (var item in li2)
                {
                    ...
                    NetworkCredential nc = new NetworkCredential("Minishopbusiness123@gmail.com", "Minishop");
        ...
        public ActionResult Create([Bind(Include = "Customerid,Customername,UserName,CustomerE_Mail,CustomerPhoneNo,Passward,Address,City,Country")] Customer cUSTOMER)
        {
            if (ModelState.IsValid)
            {
                ...
                if (a > 0)
                {
                    ...
                    smtp.Credentials = nc;
                    smtp.Send(mm);
                    TempData["Insert Message_customer"] = "<script>alert('Successfully Signed Up')</script>";
                    //ModelState.Clear();
                    return RedirectToAction("HomeView", "Home");
                }
                else
                {
                    TempData["Insert Message_customer"] = "<script>alert('Unsuccesfull SignUp Try Again!')</script>";
